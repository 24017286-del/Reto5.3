...
const URL_NOTICIAS = `https://api.thenewsapi.com/v1/news/top?api_token=${API_KEY}&locale=us&limit=3`;
...
function renderizarNoticias(articulos) {
    ...
    articulos.forEach((articulo) => {
        ...
        // TheNewsAPI usa el campo "image_url" (no "image" ni "urlToImage")
        const imagen = articulo.image_url
            ? `<img src="${articulo.image_url}" alt="${articulo.title ?? 'Imagen de la noticia'}">`
            : '';
...
async function cargarNoticias() {
    ...
    try {
        ...
        // TheNewsAPI devuelve los artículos en "data", no en "articles",
        // y no manda ningún campo "status" (eso era de NewsAPI).
