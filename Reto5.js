...
const API_KEY = 'ym96VGeteYXILCoTn6Wf9jcaVnVPgZe3MJyvXvPX';
const URL_NOTICIAS = `https://api.thenewsapi.com/v1/news/top?api_token=ym96VGeteYXILCoTn6Wf9jcaVnVPgZe3MJyvXvPX&locale=us&limit=3`;
...
function renderizarNoticias(articulos) {
    ...
    articulos.forEach((articulo) => {
        ...
        // GNews usa el campo "image", no "urlToImage" (eso era de NewsAPI)
        const imagen = articulo.image
            ? `<img src="${articulo.image}" alt="${articulo.title ?? 'Imagen de la noticia'}">`
            : '';
...
async function cargarNoticias() {
    ...
    try {
        ...
        // GNews NO devuelve un campo "status" como NewsAPI.
        // Si respuesta.ok es true, la petición fue exitosa;
        // solo queda comprobar que venga el arreglo de artículos.
